...
fn rehearsal_accounts_for_every_capability_and_preserves_every_peer() {
    ...
    let receipt =
        rehearsal::run(std::path::Path::new(&spine_root), &out).expect("rehearsal runs read-only");
    assert_eq!(
        receipt.schema_version,
        rehearsal::REHEARSAL_RECEIPT_SCHEMA_VERSION
    );
    assert_eq!(receipt.units_checked, 3, "all three retirement units");
    assert_eq!(
        receipt.capabilities_accounted, 19,
        "all nineteen adopted capabilities"
    );
    assert!(
        receipt.peers_checked >= 42,
        "all provenance peers accounted"
    );
    assert!(
        receipt.all_preserved,
        "every repo+lockfile preserved: {:?}",
        receipt.findings
    );
    assert!(
        receipt.bounded_claim.contains("bounded"),
